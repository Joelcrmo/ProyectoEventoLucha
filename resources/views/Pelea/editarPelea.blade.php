        ...
        <form id="formEditarPelea" action="{{ route('peleas.update', $pelea->ID_Pelea) }}" method="POST">
            ...
            <select id="selectParticipanteAzul" name="ID_Participante_Azul">
                @foreach ($participantesAzules as $participante)
                    <option value="{{ $participante->ID_Participante }}"
                        @if ($pelea->ID_Participante_Azul == $participante->ID_Participante) selected @endif>{{ $participante->Nombre_Par }}</option>
                @endforeach
            </select><br><br>
            ...
            <select id="selectParticipanteRojo" name="ID_Participante_Rojo">
                @foreach ($participantesRojos as $participante)
                    <option value="{{ $participante->ID_Participante }}"
                        @if ($pelea->ID_Participante_Rojo == $participante->ID_Participante) selected @endif>{{ $participante->Nombre_Par }}</option>
                @endforeach
            </select><br><br>
            ...
            <select id="selectJuez" name="ID_Juez">
                @foreach ($jueces as $participante)
                    <option value="{{ $participante->ID_Participante }}"
                        @if ($pelea->ID_Juez == $participante->ID_Participante) selected @endif>{{ $participante->Nombre_Par }}</option>
                @endforeach
            </select><br><br>
            ...
            <select id="selectArbitro" name="ID_Arbitro">
                @foreach ($arbitros as $participante)
                    <option value="{{ $participante->ID_Participante }}"
                        @if ($pelea->ID_Arbitro == $participante->ID_Participante) selected @endif>{{ $participante->Nombre_Par }}</option>
                @endforeach
            </select><br><br>
            ...
            <select id="selectVelada" name="ID_Velada">
                @foreach ($veladas as $velada)
                    <option value="{{ $velada->ID_Velada }}" @if ($pelea->ID_Velada == $velada->ID_Velada) selected @endif>
                        {{ $velada->Nombre_Vel }}</option>
                @endforeach
            </select><br><br>
